...
class DevStrat_4_6(BTgymBaseStrategy):
    ...
    def __init__(self, **kwargs):
        ...
        #self.channel_O = bt.Sum(self.data.open, - self.data.high(-1))
    ...
    def get_state(self):
        ...
        x = np.stack(
            [
                np.frombuffer(self.channel_spx.get(size=self.time_dim)),
                np.frombuffer(self.channel_yield10.get(size=self.time_dim)),
                np.frombuffer(self.channel_ir10.get(size=self.time_dim)),
